...
_hasbuilditem = true;
	...
	if (!_typeIsString && _hasRequiredTools) then {
		...
		//["conRed",format ["_needBuildItem: %1 _countIn: %2 _qty: %3", _needBuildItem, _countIn, _qty]] call diaglog;
...
if (isClass (missionConfigFile >> "SnapBuilding" >> _classname)) then {	
	...
	{					
						...
						//["conGreen",format ["_needBuildItem: %1 _countIn: %2 _num_removed : %3", _needBuildItem, _countIn, _num_removed]] call diaglog;
